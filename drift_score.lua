...
local driftTickerSound = 'https://cdn.discordapp.com/attachments/140183723348852736/1010036237324779540/bong_001.ogg'
...
function script.update(dt)
    ...
    if timePassed == 0 then
        addMessage('!!!GO BACK ON TRACK!!!', -1)
        addMessage('ReMade By Respect23', 2)
        addMessage('CTRL + D UI ON/OFF', -1)
    end
...
function addMessage(text, mood)
    ...
    if mood == 1 then
        for i = 1, 60 do
            ...
            driftAnimCount = driftAnimCount + 1
            driftAnim[driftAnimCount] = {
...
local function updateMessages(dt)
    ...
    for i = driftAnimCount, 1, -1 do
        ...
        if g.life < 0 then
            if i < driftAnimCount then
                driftAnim[i] = driftAnim[driftAnimCount]
            end
            driftAnimCount = driftAnimCount - 1
        end
    end
...
function script.drawUI()
    ...
    if UIToggle then
        ...
        local colorCombo = rgbm.new(hsv(comboColor, math.saturate(driftCombo / 10), 1):rgb(), math.saturate(driftCombo / 4))
        local colorComboDrift = rgbm.new(hsv(comboColor, math.saturate(driftCombo / 10), 1):rgb(), math.saturate(driftCombo / 4))
        ...
        ui.beginTransparentWindow('Drift', uiCustomPos + vec2(ui.getSize().x / 10, ui.getSize().y / 10), vec2(1400, 1400), true)
        ui.beginOutline()

        ui.pushStyleVar(ui.StyleVar.Alpha, 1)
        ui.pushFont(ui.Font.Title)
        ui.text('Respect23/ACDC DRIFT SERVER')
        ...
        for i = 1, #messages do
            ...
            ui.textColored(m.text, m.mood == 1 and rgbm(0, 1, 0, f) or m.mood == -1 and rgbm(1, 0, 0, f) or m.mood == 2 and rgbm(100, 84, 0, f) or rgbm(1, 1, 1, f))
        end
        for i = 1, driftAnimCount do
            local g = driftAnim[i]
            if g ~= nil then
                ui.drawLine(g.pos, g.pos + g.velocity * 4, g.color, 2)
                ui.drawLoadingText(g.pos + vec2(20, 0), ui.Font.Main, 'DRIFT', g.color, ui.Anchor.Left, ui.Anchor.Center, g.life * 100)
            end
        end
        ui.popFont()
